...
- https://leanpub.com/simple-haskell-book/read
...
* 08 | Real World Haskell                        | Bryan O'Sullivan
...
instance Show Color where
  ...
  show Blue = "azul"
    #+end_src
* 10 | Seven Languages in 7 weeks                | Bruce A. Tate
** Haskell
...
| list  | [T]   | head/tail |                   |   |
* 20 | Finding Success and Failure               | Julie Moronuki
** Preface
...
- Bool has only 2 values, also called *constructors*, True and False
...
*** Example: pattern matching on Bool values to return Maybe values
  #+begin_src haskell
...
- New Stack project: where *simple* is the most minimal of the templates that stack provides
  $ stack new validation-book simple
- Project aware REPL that give us their dependencies:
  $ stack repl
- The module name is *Main* because an /executable/ must always be named *Main*
** 4 The =Maybe= Monad
- Solution in this capter
  - will use the bind operator (>>=)
  - won't tell you which of the validtion rule failed
...
- A *typeclass* defines a set of /generic functions/ that work
  with a set of types.
- A =class= declaration gives only the type signatures in their
  most polymorphic forms.
...
main =
  ...
  #+end_src
- main rewritten from *do* to bind, without explicit argument passing
  #+begin_src haskell
main :: IO ()
...
- Where *Right b* is similar to *Just a* from Maybe
  Where *Left a* can carry data unlike *Nothing* form Maybe
*** The =Either= Monad
#+begin_src haskell
...
- ~type aliases~ do not offer *safety*, it only gives you another name for the same type
  ...
  #+end_src
- ~newtype~
  it has a *data constructor* that is named differently from the underlying type's constructor.
  #+begin_src haskell
    newtype Password = Password String
      deriving Show
  #+end_src
- differences of *newtype* with *data*
  1) /newtype/ may only have single unary constructor
  2) /newtype/ optimizations due 1)
  3) /newtype/ offers a cheap coercion, between newtype and the underlying type,
     /data/ does not
...
- When to use >>= or do
  1) *Too many lambdas*, if it seems like you're writing way too many lambdas, use *do*
  2) *Too many variables*, that get introduced one line, only to get used on the next, use >>=
  3) Or both
     #+begin_src haskell
       do
         x1 <- a1 >>= f1 >>= f2
...
- A ~product type~, the conjuction (and) of two or more types
...
- Applicative typeclass operator (<*>), aka ~tie-fighter~ or ~apply~ or ~app~
  Allows us for function application in the presence of some outer type structure (aka context)
...
makeUser name password =
  User <$> validateUsername name
    <*> validatePassword password
#+end_src
*** Using Applicative with checkAnagram
#+begin_src haskell
checkAnagram :: String -> String -> String
...
- refactor from =Either= to =Validation=
  ...
  - their Applicative instance is different, we an accumulate errores on the Left1
...
- Since functions chained together with *applicative* (<*>) can be evaluated independently:
  - we can /accumulate/ the errors from several function applications.
  - /concatenate/ them using the underlying *semigroup*
  - and /return/ as many errors as there are.
- err needs a semigroup constraint
  #+begin_src haskell
    instance Semigroup err => Applicative (Validation err)
  #+end_src
- We will still use ~Error~ we created before,
  - while it is a *newtype* from String, and String is a semigrup
  - we don't inherit the instances of the underlying type by default
*** Adding a dependency
- add a executable stanza, into .cabal
  #+begin_src yaml
    execultable validation
      hs-source-dirs:   src
      main-is:          Main.hs
...
*** An Error semigroup
*** Using Applicative
* 22 | Practical Haskell                         | Alejandro Serrano Mena
...
* 22 | Sockets & Pipes                           | Chris Martin
- cabal environment
  https://hackage.haskell.org/package/sockets-and-pipes
  https://github.com/joyofhaskell/sockets-and-pipes
- someone's code https://github.com/brhutchins/sockets-and-pipes/blob/main/Book.hs
** Preface
- Libraries: bytestring, text, sockets, network, attoparsec
...
#+begin_src yaml
  ...
  library # no colon (:), nor this comment
    default-language:   Haskell2010
...
#+end_src
** 1 Handles
- ~multiplexing~ is allowing many "tenants" to share the same facilities without interfering with one another
- IO =~ system calls to the OS
- a "handle" is the identifier for the OS<->Process "conversation
  - windows: file handle
  - linux: file descriptor
- IO.(openFile/hClose/hPutStrLn/putStrLn)
- what could go wrong?
  1) IO Exceptions: eg: disk is full
  2) Asynchronous exceptions: eg: someone throws an interrupt (Control.Exception.throwTo function)
- the ~T~ at the end of a type stands for *transformer*
  wherever there are monad transformers we find an abundance of polymorphism
- ~ResourceT~ is a monad transformer
  if *m* is a monad *mT* is a monad as well
- ~ResourceT IO~
  ...
  - anything that you can do in ~IO~ can also be done in ~ResourceT IO~
    by lifting the ordinary ~IO~ inot a resource-safety-agumented ResourceT IO context
...
- This tell us wht is the general structure of a program using the type will look like.
*** writeGreetingFile
#+begin_src haskell
...
** 10 Recap
* 23 | Effective Haskell                         | Rebecca Skinner
** DONE Introduction
...
- wrapped inside of a StringParser,
  will make it somewhat easier to work with (?)
...
** 05 Creating And Structuring Haskell Projects
** 06 Type Classes
** 07 Understanding =IO=
