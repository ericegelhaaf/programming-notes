...
- https://github.com/sjl/cl-losh/blob/master/src/iterate.lisp
...
- Accumulation:
  ...
  - unioning: appending+unique
  - nunioning
  - accumulating: accumulation builder, ex: (accumulating lst by #'union))
- Reduction:
  - multiplying:
  - reducing: a reduccion builder, ex:
    (reducing d by #'/ initial-value 0)
    OR with a macro
     #+begin_src lisp
...
** gtwiwtg "naive generators"
   https://github.com/cbeo/gtwiwtg
** Series Package Appendix https://www.cs.cmu.edu/Groups/AI/html/cltl/clm/node347.html
