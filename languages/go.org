* Videos
- Gaming with GO https://www.youtube.com/user/Velovix
...
- https://github.com/astaxie/build-web-application-with-golang/blob/master/en/09.3.md
...
func (l +lookup) process() {
  ...
  #+end_src
* Video: Practical Go: Real world advice for writing maintainable Go programs / Dave Cheney (2019)
...
1:18:32
* Video: Will contracts replace interfaces? / Francesc Campoy (2019)
...
- ServMux() a server multiplexer, collects several handlers into one
  - We register the handlers, *.Handle()* passing the path and the method to handle the path
...
- In Go, a webservice relies on:
  ...
  - Multiplexer
- Client Timeouts:
  IdleTimeout: control keepalive timeout
  ReadHeaderTimeout:
  ReadTimeout: time the client has to send both header and body
  WriteTimeout: time it has the client to read the server reply
- TLS
  &http.Server{}.Serve()
                .ServeTLS()
*** Handlers
  ...
  - http.Server will close the request body, but it won't drain it
    #+begin_src go
    // Inside a Handler
    ...
    #+end_src
  - *html/template* package to escape HTML code
- Testing with *net/http/httptest* package
  ...
  httptest.NewRecorder returns a pointer to httptest.ResponseRecorder
      use the .Result() methos to return a pointer to http.Response
...
- ~internal/log/store.go~
  ...
  - .ReadAt()
    wrapper over .File.ReadAt() that calls buf.Flush()
  - .Close()
    wrapper over .File.Close() that cals to buf.Flush()
